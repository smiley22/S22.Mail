...
namespace S22.Mail {
	...
	internal class MessageReader {
		...
		/// <summary>
		/// Reads and processes the message data from the stream and constructs
		...
		public MailMessage ReadMailMessage() {
		...
		private NameValueCollection ReadMailHeader() {
			...
			while ((response = reader.ReadLine()) != String.Empty) {
		...
		private MIMEPart[] ReadMultipartBody(string boundary, bool nested = false) {
			...
			while (response.StartsWith(s_boundary)) {
				...
				while (!(response = reader.ReadLine()).StartsWith(s_boundary))
